...
const Project = () => {
  ...
  const loadData = async () => {
    const response = await axios.get(
      process.env.REACT_APP_API_URL + '/dashboard/project/get'
    ); //returns data
  ...
  const deleteProject = (id) => {
    axios
      .delete(process.env.REACT_APP_API_URL + `/dashboard/project/remove/${id}`)
      .then((response) => {
        if (response.data === 'OK') {
  ...
  const transform = (item) => {
    return {
      ...
      action: [
        <Link to={`/dashboard/project/update/${item.projectID}`}>
          <button
        ...
        <MDBModal show={basicModal} setShow={setBasicModal} tabIndex="-1">
          <MDBModalDialog>
            <MDBModalContent>
              ...
              <MDBModalBody>
                Are you sure you want to delete Project ID: {deleteId} ?
              </MDBModalBody>
              <MDBModalFooter>
                <MDBBtn
                  className="btn-dark"
